...
class RelativeGlobalAttention(torch.nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, **kwargs):
        ...
        E = self._get_left_embedding(self.len_q, self.len_k).to(q.device).to(q.dtype)
        ...
        if attn_mask is not None:
            print(attn_mask.size(), logits.size())
            attn_mask = attn_mask.unsqueeze(0)
            logits += (attn_mask.to(torch.int64) * -1e9).to(logits.dtype)
...
class MultiheadCoverageAttention(MultiheadAttention):
    """Multi-headed attention.
