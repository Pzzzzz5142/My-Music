...
class RelativeMultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = quant_noise(nn.Linear(self.kdim, embed_dim, bias=bias), 0.0, 8)
        self.v_proj = quant_noise(nn.Linear(self.vdim, embed_dim, bias=bias), 0.0, 8)
        self.q_proj = quant_noise(nn.Linear(embed_dim, embed_dim, bias=bias), 0.0, 8)

        if bias:
            self.in_proj_bias = Parameter(torch.Tensor(3 * embed_dim).cuda())
        else:
            self.register_parameter("in_proj_bias", None)
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias=bias).cuda()

        if add_bias_kv:
            self.bias_k = Parameter(torch.Tensor(1, 1, embed_dim).cuda())
            self.bias_v = Parameter(torch.Tensor(1, 1, embed_dim).cuda())
        else:
        ...
        self.relative_position_keys = Parameter(
            torch.Tensor(2 * self.max_relative_length + 1, self.head_dim).cuda()
        )
        if not self.k_only:
    ...
    def forward(
        ...
        if incremental_state is not None:
            ...
            if "prev_key" in saved_state:
                ...
                if static_kv:
                    assert kv_same and not self.qkv_same_dim
                    key = value = None
        else:
            saved_state = None
