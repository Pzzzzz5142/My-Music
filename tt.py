...
from pt.model import MusicTransformer, SmoothCrossEntropyLoss

from pt.utilities import *
...
def main():
    ...
    def load_dataset(split, **kwargs):
        """Load a given dataset split.
        Args:
            split (str): name of the split (e.g., train, valid, test)
        """
        data_path = "data-bin/cl"
        dictionary = Dictionary.load(os.path.join(data_path, "dict.txt"))
        dataset = data_utils.load_indexed_dataset(
            os.path.join(data_path, split), dictionary, None, False
        )
        if dataset is None:
            raise FileNotFoundError(
                "Dataset not found: {} ({})".format(data_path, split)
            )
        dataset = maybe_shorten_dataset(dataset, split, "", "random_crop", 2048, 1,)
    ...
    train_dataset, val_dataset, test_dataset = [
        load_dataset(i) for i in ["train", "valid", "test"]
    ]
    ...
    """
    indices = train_dataset.ordered_indices()

    batch_sampler = train_dataset.batch_by_size(
        indices, max_tokens=4096, max_sentences=None, required_batch_size_multiple=8,
    )
    train_loader = iterators.EpochBatchIterator(
    ...
    ##### Optimizer #####
    opt = Adam(
        model.parameters(), lr=lr, betas=(ADAM_BETA_1, ADAM_BETA_2), eps=ADAM_EPSILON
    )
