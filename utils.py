...
def item2event(groups):
    ...
    for i in range(len(groups)):
        ...
        n_downbeat += 1
        events.append(
            Event(name="Bar", time=None, value=None, text="{}".format(n_downbeat))
        )
            ...
            if item.name == "Note":
                ...
                events.append(
                    Event(
                        ...
                        text="{}/{}".format(
                            item.velocity, DEFAULT_VELOCITY_BINS[velocity_index]
                        ),
                    )
                )
                # pitch
...
# WRITE MIDI
#############################################################################################
def word_to_event(words, word2event=None):
    events = []
    if word2event == None:
        for word in words:
            event_tmp = word.split("_")
            event_value = event_tmp[-1]
            event_name = " ".join(event_tmp[:-1])
